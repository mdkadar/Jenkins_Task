    ...
    stages {
        stage ('Installing updates') {
            steps {
                ...
                //bat "powershell.exe $arg = Get-WmiObject Win32_Product -Filter "Name like 'Octopus Deploy Server'" | Select-Object -ExpandProperty Version"
                //bat "powershell.exe If($arg -ne "2021.1.7665") { Write-Host "Not upto date." } else { Write-Host "upto date." }"
